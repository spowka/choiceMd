...
class Users extends Component {
  ...
  fetchData(pageNumber, user) {
    axios
      .get(`/api/v1/users?${this.getSearchQueryString(pageNumber, user)}`)
      .then(response => {
        this.setState({
                  ...
                  <tbody>
                    {this.state.results.map(r => (
                      <tr key={r.id_user}>
                        ...
                        <td>{r.email}</td>
                        <td>{r.is_activated ? 'YES' : 'NO'}</td>
                        <td className="admin-table-actions">
                          <div className="btn-group">
                            <Link
                              to={'/admin/user/edit/' + r.id_user}
                            ...
                            <a
                              className="btn btn-pill btn-hero btn-violet"
                              data-toggle="modal"
                              data-target={`#deleteModal_${r.id_user}`}
                            >
...
export default withGlobalContext(Users);
