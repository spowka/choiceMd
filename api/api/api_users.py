...
def api_users_get():    
    ...
    sort_by, results_page, results_page_size = get_select_params(request, allowed_sort_by=["name", "role", "email", "is_activated", "date_activated"])

    (name, ) = parse_url_params(request, ["name"])

    users, result_count = User.get_users(name = name,
                                         sort_by = sort_by,
    ...
    json_str = jsonpickle.encode({"current_page": results_page, "page_size": results_page_size, "total_count": result_count, "total_pages": total_pages, "sort_by": sort_by, "results": sanitized_users}, unpicklable=False)
...
def api_users_post():
    ...
    is_json_ok, data, json_error = parse_json(request.data, ["id_user_role", "email", "password", "name", "is_activated"])
...
def api_users_id_patch(id_user):
    ...
    is_json_ok, data, json_error = parse_json(request.data, ["name"])
...
def api_users_registration_post():
    is_json_ok, data, json_error = parse_json(request.data, ["email", "password", "name", "client_id"])
    ...
    user = User()
    user.id_user_role = UserRole.ID_USER_ROLE_REGULAR
    ...
    send_mail("perooo@gmail.com", [user.email], "ChoiceMD registration", "Click here to activate your ChoiceMD account: <a href='http://choicemd.com/activation?code={0}'>Activation link</a>".format(user.activation_code))

    return Response(response=json_str, status=201, mimetype="application/json")


@api_users.route("/users/activation", methods=["POST"])
def api_users_activation_post():
    is_json_ok, data, json_error = parse_json(request.data, ["email", "code"])

    if not is_json_ok:
        return Response(json_error, 400)

    user = User.activate(data["email"], data["code"])
...
def api_users_password_post():
    ...
    if data["action"] == "reset":
        ...
        mail.send_mail(current_app.config['MAIL_USERNAME'], [user.email], "ChoiceMD password change", "Click the following link: <a href='http://choicemd.com/reset-password?code={0}'>Reset password</a>".format(code))
